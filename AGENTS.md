...
- `mpc_v2/`：重建后的 deterministic/replay MPC v1 验证包，保留前后数据流，用于固定输入下的 TES MPC 算法复核；不是 EnergyPlus 在线耦合入口。
...
- MPC 算法验证、固定输入 MPC 情景、SOC-neutral 复核或 MPC 输出文件问题：优先查看 `mpc_v2/`，并遵守 `mpc_v2/AGENTS.md`。
...
- 根目录 `CHANGELOG.md` 是项目版本管理文件。
- 每次形成可复现版本、提交代码、保存仿真结果、完成验证矩阵或更新实验结论时，必须同步检查并更新 `CHANGELOG.md`。
- 每个版本条目必须记录：
  - 版本号与日期
  - Git commit
  - 本版本代码、模型、输入和文档变更范围
  - 验证命令和测试结果
  - 运行结果位置，例如 `results/<version_or_topic>_<YYYYMMDD>/`
  - 运行结果简述，必须包含核心指标、是否达到目标和结论边界
  - 已知限制和下一步
  - 是否影响论文；如影响，是否已更新 `thesis_draft.tex` 或 `references.bib`
- 如果本版本没有新增仿真或验证结果，必须明确写：
  `运行结果位置：无新增结果`
  并说明原因。
- 不要把版本历史正文写进 `AGENTS.md`；`AGENTS.md` 只定义规则，实际版本记录写入 `CHANGELOG.md`。
...
- 如果用户明确提到 MPC 算法、固定点位/固定环境输入、SOC-neutral 复核或 MPC 数据流，默认目标目录是 `mpc_v2/`。
