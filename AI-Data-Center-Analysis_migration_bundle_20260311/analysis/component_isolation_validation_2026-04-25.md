...
- 2026-04-29 M2-F1 更新：agent 当前暴露 **4 维** `[CT_Pump_DRL, CRAH_T_DRL, Chiller_T_DRL, TES_DRL]`；底层 HVAC+TES full action 仍保留 5 维，`CRAH_Fan_DRL` 固定插入为 1.0，不再由 agent 控制。
...
### 当前 M2 阶段：无阻塞
本次验证已确认底层 action 通道物理有效。M2-F1 训练阶段暂固定 Fan=1.0，仅让 agent 控制 CT_Pump / CRAH_T / Chiller_T / TES_valve；完整 HVAC 联合控制后移。
