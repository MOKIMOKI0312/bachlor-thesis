...
class ModelJSON(object):
    ...
    def __init__(
        ...
        self._json_path = os.path.join(
            self.pkg_data_path, 'buildings', random.choice(self.json_file))
        ...
        self._stat_path=self._weather_path.split('.epw')[0] + '.stat'

        with open(self._stat_path, "r", encoding="ISO-8859-1") as file:
            for line in file:
                if "annual (standard) wind speed" in line.lower():  # Case-insensitive search
                    match = re.search(r"([\d.]+)", line)  # Extract first numeric value
                    if match:
                        self.wind_speed = float(match.group(1))  # Convert to float
    ...
    def adapt_building_to_epw(
        ...
        if "Generator:WindTurbine" in self.building:
            for key, value in self.building["Generator:WindTurbine"].items():
                self.building["Generator:WindTurbine"][key]["annual_local_average_wind_speed"]=self.wind_speed
    ...
    def update_evaluation_weather_path(self, episode_value) -> None:
        ...
        # JSON
        self._json_path = os.path.join(
            self.pkg_data_path, 'buildings', self.json_file[episode_value])
    ...
    def apply_weather_variability(
        ...
        filename = os.path.basename(self._weather_path)
...
# ---------------------------------------------------------------------------- #
    ...
    def _get_working_folder(
        ...
        """Create a working folder path from path_folder using base_name, returning the absolute result path.
           Creates numbered folders like run-001 or episode-001.

        Args:
            path_folder (str): Path when working dir will be created.
            prefix (str, optional): Folder prefix used to name the new folder inner path_folder. Defaults to 'run-'.
        ...
        for folder_name in os.listdir(directory_path):
            if not os.path.isdir(os.path.join(directory_path, folder_name)):
                continue
            match = re.fullmatch(rf'{re.escape(prefix)}(\d+)', folder_name)
            if match:
        ...
        working_dir = os.path.join(directory_path, f'{prefix}{experiment_id:03d}')
        return working_dir
    ...
    def _rm_past_history_dir(
        ...
        cur_dir = os.path.basename(episode_path)
        match = re.fullmatch(rf'{re.escape(prefix)}(\d+)', cur_dir)
        if match is None:
            return
        ...
        if cur_dir_id - self.max_ep_store > 0:
            ...
            rm_dir_full_name = os.path.join(
                ...
                f'{prefix}{rm_dir_id:03d}',
            )
            rmtree(rm_dir_full_name)

    def _infer_run_kind(self, env_name: str) -> str:
        env_name = env_name.lower()
        if 'probe' in env_name:
            return 'probe'
        if 'train' in env_name:
            return 'train'
        if 'eval' in env_name or 'compare' in env_name:
            return 'eval'
        return 'run'
