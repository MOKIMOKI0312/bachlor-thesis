...
class EplusEnv(gym.Env):
    ...
    def __init__(
        self,
        building_file: Union[str, List[str]],
        ...
        max_ep_data_store_num: int = 100,
        env_name: str = 'eplus-env-v1',
        grid_flag: int = 0,
        config_params: Optional[Dict[str, Any]] = None,
        evaluation_flag: int = 0,
        wind_def: float = 0,
        solar_def: float = 0
    ):
        ...
        # ---------------------------------------------------------------------------- #
        random.seed()
        # randomly select one building File if a list is provided
        # Building file(s) (str or List of JSon's)
        if isinstance(building_file, str):
            self.building_file = [building_file]
        else:
            self.building_file = building_file

        print(self.building_file)
        self.bulding_len=len(self.building_file)

        random.seed()
        # randomly select one EPW File if a list is provided
        ...
        # check actions for AI Server Control
        self.Server_Control = 0
        if 'Server_RL' in self.actuators:
            self.Server_Control = 1
        ...
        #                               Observation Space                              #
        # ---------------------------------------------------------------------------- #
        obs_size = len(time_variables) + len(variables) + len(meters)
        # EnergyPlus variables span very different magnitudes depending on the
        # selected building and wrappers. Leave the bounds effectively unbounded
        # so Gym's passive checker does not reject valid observations.
        ...
        # ---------------------------------------------------------------------------- #
        #                        Define Cuts if renewable curtailments are used        #
        # ---------------------------------------------------------------------------- #
        self.wind_cut=0
        self.solar_cut=0
        self.evaluation_flag=evaluation_flag
        self.wind_def=wind_def
        self.solar_def=solar_def
        self.SoC = 0
        self.SoC_low = 0
        self.SoC_high = 0
    ...
    def reset(self,
        ...
        # get weather path
        if self.evaluation_flag ==0:
            self.model.update_weather_path()
        else:
            self.model.update_evaluation_weather_path((self.episode-1)%self.evaluation_flag)
        used_weather_path=self.model.weather_path
        ...
        if self.grid_flag:
            GRID_MAP = {
                'Dulles': 'NorthVirginia', 'Dallas': 'Dallas', 'Sydney': 'Sydney',
                'Frankfurt': 'Frankfurt', 'Singapore': 'Singapore', 'Lulea': 'Lulea',
                'San.Francisco': 'SiliconValley', 'Des.Moines': 'DesMoines',
                'Omaha': 'Omaha', 'New.York': 'NewYork',
                'Nanjing': 'Singapore', 'Xuzhou': 'Singapore', 'Yinchuan': 'Singapore',
            }
            grid_name = None
            for key, name in GRID_MAP.items():
                if key in used_weather_path:
                    grid_name = name
                    break
            if grid_name is None:
                grid_name = 'Singapore'
            grid_price = np.loadtxt(PKG_DATA_PATH + '/Grid Data/' + grid_name + '.csv', delimiter=',')
            grid_CF = np.loadtxt(PKG_DATA_PATH + '/Grid Data/' + grid_name + '_CF.csv', delimiter=',')
            if self.evaluation_flag == 0:
                self.grid_price = grid_price[:, 0]
                self.grid_CF = grid_CF[:, 0]
            else:
                self.grid_price = grid_price[:, 1]
                self.grid_CF = grid_CF[:, 1]
        ...
        if 'grid_price' in obs:
            obs['grid_price']=self.grid_price[self.timestep-1]
            obs['grid_CF']=self.grid_CF[self.timestep-1]
    ...
    def step(self,
        ...
        if self.energyplus_simulator.simulation_complete:
            ...
            try:
                ...
                obs = self.last_obs
                info = self.last_info
        used_path=self.model.weather_path
        # ---------------------------------------------------------------------------- #
        #                               Import Grid Data                              #
        # ---------------------------------------------------------------------------- #

        if 'grid_price' in obs:
            if self.timestep<8761:
                obs['grid_price']=self.grid_price[self.timestep-1]
                obs['grid_CF']=self.grid_CF[self.timestep-1]
            else:
                obs['grid_price']=self.grid_price[8759]
                obs['grid_CF']=self.grid_CF[8759]
