...
class EnergyPlus(object):
    ...
    def __init__(
        ...
        self.special_variable_readers = {
            'outdoor_DFI': self._get_today_diffuse_solar,
            'outdoor_DRI': self._get_today_direct_solar,
        }
        ...
        # grid factors
        self.grid_price: Optional[float] = None
        self.grid_CF: Optional[float] = None
    ...
    def _collect_obs_and_info(self, state_argument: int) -> None:
        ...
        self.next_obs = {
            ...
            **{
            ...
            # variables sourced directly from the weather API
            **{
                key: reader(state_argument)
                for key, reader in self.special_variable_readers.items()
    ...
    def _process_action(self, state_argument: int) -> None:
        ...
        timeout = 10
        try:
            next_action = self.act_queue.get(timeout=timeout)
        except (Full, Empty):
            return
        ...
        if self.exchange.api_data_fully_ready(
            ...
            if self.var_handlers is None or self.meter_handlers is None or self.actuator_handlers is None:
                ...
                self.var_handlers = {
                    ...
                    if key not in self.special_variable_readers
                }
    ...
    def _get_today_diffuse_solar(self, state_argument: int) -> float:
        return self.exchange.today_weather_diffuse_solar_at_time(
            state_argument,
            self.exchange.hour(state_argument),
            self.exchange.zone_time_step_number(state_argument))

    def _get_today_direct_solar(self, state_argument: int) -> float:
        return self.exchange.today_weather_beam_solar_at_time(
            state_argument,
            self.exchange.hour(state_argument),
            self.exchange.zone_time_step_number(state_argument))
