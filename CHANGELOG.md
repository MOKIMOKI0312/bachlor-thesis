...
## v0.3.1-mpc-agent-rules - 2026-05-07

### Git

- Commit: `45f50de0` 为当前工作树基线；本版本文档规则变更尚未提交。
- 状态：仅更新 agent 工作规则；未新增仿真、验证矩阵或冻结结果。

### Scope

本版本明确 MPC 算法固定验证情景的入口和写入规则：

```text
root AGENTS trigger rule
+ mpc_v2 local AGENTS rules
+ changelog record for documentation-only update
```

### Changes

- `AGENTS.md`：增加 `mpc_v2/` 作为 synthetic/replay MILP-MPC 验证框架的路径说明；明确 MPC 算法验证、固定点位/固定环境输入、SOC-neutral 复核和 TOU/DR synthetic/replay 矩阵优先进入 `mpc_v2/`。
- `mpc_v2/AGENTS.md`：新增 MPC 固定验证情景写入方法、使用边界、输出目录、验收指标和结果解释规则。
- 未修改 `docs/project_management/AGENTS.md`，因为本次规则属于代码工作区入口与 MPC 验证夹具，不属于项目管理或论文目录规则。

### Validation

Document-level validation command:

```powershell
git diff -- AGENTS.md CHANGELOG.md
Get-Content -Path mpc_v2/AGENTS.md
git status --short -- AGENTS.md CHANGELOG.md mpc_v2/AGENTS.md
```

Result:

```text
文档结构、触发词、输出目录和使用边界已检查。
```
...
原因：本版本只更新 agent 协作规则和 MPC 固定验证情景的文档化写入方法，未执行仿真。
...
- 未更新 `docs/project_management/毕业设计论文/thesis_draft.tex`。
- 未更新 `docs/project_management/毕业设计论文/references.bib`。
- 原因：本版本不改变模型描述、实验结果、输入数据、技术结论或引用事实，只改变 agent 工作规则。

### Known Limitations

- `mpc_v2/AGENTS.md` 只定义固定验证情景规则；实际代码入口、scenario YAML、解析夹具和测试仍需在后续实现。
- 该固定验证情景未来只能用于 MPC 算法验算，不能替代 TOU/DR 主线收益矩阵或 EnergyPlus online co-simulation。
