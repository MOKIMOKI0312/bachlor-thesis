...
本文面向南京数据中心冷却场景，构建包含冷机模式、冷水蓄冷系统、光伏出力、分时电价和机房温度约束的控制导向模型，并设计基于混合整数线性规划的模型预测控制方法。该方法在滚动预测时域内优化冷机出力、TES 充放冷功率、电网购电功率和 PV 弃电功率，同时显式考虑冷机 mode 约束、TES SOC 约束、充放冷互斥约束、温度安全约束和终端状态要求。当前可复现 24 h nominal smoke 结果表明，相比公平 no-TES baseline，MILP-MPC 总成本下降约 10.93\%，冷站能耗下降约 3.17\%，温度越界为零；但 nominal case 中 TES 主要表现为充冷缓冲，尚未形成有效放冷、削峰或 PV 消纳提升。因此，本文将该结果作为冷机 + TES 联合 MPC 可运行性的阶段性证据，并在讨论中明确其模型边界。
...
This thesis develops a control-oriented framework for a data-center cooling system with chiller plant modes, chilled-water thermal energy storage, photovoltaic generation, and time-of-use electricity prices. A mixed-integer linear model predictive controller is formulated to optimize chiller load, TES charging and discharging, grid import, and PV curtailment over a receding horizon while enforcing temperature, SOC, and mode-switching constraints. Reproducible 24 h nominal smoke results show that the MILP-MPC reduces total cost by about 10.93\% and cold-station energy by about 3.17\% relative to a fair no-TES baseline, with zero temperature violation. However, nominal TES behavior is still dominated by charging rather than discharge, and peak shaving or PV-spill reduction is not yet demonstrated.
...
本章记录当前 \texttt{chiller\_tes\_mpc\_v1\_20260505} 版本的可复现闭环仿真结果。结果文件保存于 \texttt{results/chiller\_tes\_mpc\_v1\_20260505/}，对应 smoke run 和 validation matrix 均采用 15 min 控制步长、96 个闭环步和 192 步滚动预测时域。当前结果用于说明冷机 + TES 联合 MPC 代码路径已经可运行，并不等同于最终工程部署结论。

\section{Nominal case 下的 MPC 调度行为}

Nominal case 首先用于检查 MPC 是否产生合理行为，而不是直接证明经济收益。本版本中，MPC 能够调用冷机 mode、维持机房温度约束，并在低价或 PV 充足时段将部分冷量转入 TES；但在 24 h nominal case 中，TES 未形成放冷动作。这说明当前代理模型和滚动终端条件仍存在 SOC hoarding，TES 的名义高价放冷价值尚未充分体现。
...
对当前结果的回答是：控制器会充冷，SOC 保持在物理边界内，温度约束满足；但 nominal case 未出现 TES 放冷，因而不能声称 TES 已经在基准场景中完成有效削峰填谷。

\section{与 no-TES baseline 的对比}

\begin{table}[htbp]
  \centering
  \caption{no-TES baseline 与 MILP-MPC 对比结果}
  \begin{tabular}{lll}
    \toprule
    指标 & no-TES & MILP-MPC \\
    \midrule
    Total cost & 4073.02 & 3627.88 \\
    Grid import (kWh) & 49648.43 & 47442.19 \\
    Peak grid demand (kW) & 3090.00 & 3090.00 \\
    PV spill (kWh) & 342.96 & 342.96 \\
    Cold-station energy (kWh) & 69649.07 & 67442.83 \\
    Temp violation degree-hours & 0.00 & 0.00 \\
    \bottomrule
  \end{tabular}
\end{table}

该对比显示，MPC 相对 no-TES baseline 的总成本下降约 10.93\%，冷站购电量下降约 4.44\%，冷站能耗下降约 3.17\%。但峰值功率未下降，PV spill 未下降。因此，当前版本可以支持“冷机 + TES MPC 改善名义成本和冷站能耗”的结论，不能支持“已实现削峰或提升 PV 消纳”的结论。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llll}
    ...
    TES cycles & - & 0.69 & 0.00 \\
    \bottomrule
  \end{tabular}
\end{table}

RBC 在本版本中产生了明显 TES 充放冷，等效循环为 0.69，但其 PV spill 高于 no-TES 和 MPC，且总成本仍高于 MPC。MPC 的成本最低，但 TES 等效循环为 0，说明 MPC 的经济收益主要来自冷机 mode 和冷站功率调度，而不是 nominal case 下的 TES 放冷。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llll}
    ...
    \midrule
    PV scale 1.0 & 3627.88 & 342.96 & 0.00 \\
    PV scale 2.0 & 2693.73 & 12114.02 & 0.00 \\
    PV scale 3.0 & 2426.58 & 29337.78 & 0.00 \\
    \bottomrule
  \end{tabular}
\end{table}

该结果说明，提高 PV 规模可以降低购电成本，但也会显著增加弃光，因为当前优化边界是冷站功率表计，许多时段冷站负荷小于 PV 出力。后续若要证明 TES 提高 PV 消纳，需要进一步调整 TES 放冷价值、冷站负荷边界或 PV 场景。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llll}
    ...
    \midrule
    hot & 4952.51 & 0.28 & 0.19 \\
    mild & 1689.16 & 0.00 & 0.00 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  \centering
  \caption{电价与需求电费敏感性结果}
  \begin{tabular}{llll}
    \toprule
    Tariff case & Cost saving & TES cycles & Peak grid reduction \\
    \midrule
    tariff base & 3627.88 & 0.00 & 0.00 \\
    tariff high & 5434.34 & 0.00 & 0.00 \\
    demand high & 5018.38 & 0.00 & 0.00 \\
    \bottomrule
  \end{tabular}
\end{table}

电价和需求电费场景目前没有带来 TES 放冷或削峰。这说明当前 mode-based 代理模型虽然能够形成可解 MILP，但还需要进一步校准低 PLR 惩罚、SOC 目标和需求峰值项，才能使 TES 在 nominal 或 demand-charge 场景中承担削峰功能。

\section{温度约束与 SOC 约束分析}

温度和 SOC 约束决定控制策略是否工程可接受。Nominal smoke 中 no-TES、RBC 和 MPC 的温度越界 degree-hours 均为 0。Hot 场景中 MPC 出现 0.19 degree-hours 温度越界和 1 次 fallback，说明高温扰动下当前冷站容量、温度代理和求解器设置仍需进一步调校。

\section{求解性能分析}

MPC 需要在每个控制步反复求解 MILP，因此求解性能是工程可行性的重要指标。Nominal MPC smoke 的 p95 求解时间为 2.27 s，fallback count 为 0，feasible rate 为 1.00；这说明当前 192 步预测域和 mode-based MILP 在本地 SciPy/HiGHS 管线上具备基本实时可行性。
...
本版本最重要的边界是：MPC nominal case 的经济收益主要来自冷机 mode 调度和冷站功率边界重构，而不是 TES 放冷。TES 在 nominal case 只充不放，说明当前代理模型存在 SOC hoarding 和房间热惯性替代 TES 的问题。后续需要引入更准确的冷量需求模型、SOC 持有成本、需求峰值结算窗口或更强的末端策略，才能证明 TES 的削峰填谷价值。
...
当前闭环结果表明，冷机 + TES MPC 相比公平 no-TES baseline 可以降低 nominal case 的总成本和冷站能耗，并保持温度约束安全；但 nominal case 未形成 TES 放冷、削峰或 PV 消纳提升。因此，当前版本已经达到“模型清楚、变量清楚、闭环可运行、baseline 可比较”的阶段目标，但尚未达到“证明 TES 在 nominal case 中显著削峰填谷”的最终目标。
