...
为避免把单一 18 MWh\_th TES 参考值写成最优容量，本文新增三地点全年 PV--TES 技术容量矩阵。矩阵使用南京、广州和北京真实 EPW 驱动的 EnergyPlus 年度 no-control 负荷/天气边界，PV 出力来自 PVGIS v5.3 20 MWp 曲线并按装机线性缩放，电价统一采用江苏 2025 TOU 曲线。主扫描为 $PV=\{0,10,20,40,60\}$ MWp、$TES=\{0,9,18,36,72\}$ MWh\_th、尖峰电价上浮 $\delta=\{0,0.2\}$，共 150 个全年场景，每个场景包含 35040 个 15 min 控制步。

审计结果显示全量矩阵 150 个场景均完成，P0 错误为 0；唯一 P1 警告是 15 个 9 MWh\_th 小 TES 场景出现轻微负尖峰电价抑制，说明容量过小且回充惩罚可能超过尖峰放冷收益。三地点最大尖峰电价抑制率分别约为北京 0.587、广州 0.528、南京 0.482；20 MWp PV + 72 MWh\_th TES 在当前技术规则下分别取得北京 0.439、广州 0.365、南京 0.352 的尖峰抑制率，同时 PV 自消纳率分别约为 0.829、0.947、0.912。年度绝对峰值削减接近零，说明当前 EnergyPlus 边界下主收益并非年度峰值迁移，而是尖峰窗口购电抑制和 PV 自消纳改善。

因此，本节只给出运行层面的 technical recommended capacity range：20 MWp PV 可作为三地点共同参考装机，TES 容量宜落在 36--72 MWh\_th 的高容量区间，其中 72 MWh\_th 在当前规则下更接近最大尖峰抑制效果。该结论不包含 CAPEX、LCOE、NPV 或投资回收约束，不能表述为 economic optimum。
