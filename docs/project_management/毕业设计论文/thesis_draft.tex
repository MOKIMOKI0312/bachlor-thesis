...
\title{面向数据中心冷却系统的 TES-PV-TOU Relaxed MPC 控制方法研究}
\author{作者姓名}
\date{\today}
...
本文面向南京数据中心冷却场景，构建包含冷站、冷水蓄冷系统、光伏出力和分时电价的 Kim-lite 控制导向模型，并设计 relaxed model predictive control 方法。主实验中冷站模式变量采用连续松弛处理，用于近似聚合冷站的连续调节能力，并将研究重点放在 TES 在 TOU、critical peak 和 peak-cap 场景下的调度机制。当前可复现结果表明，\texttt{paper\_like\_mpc\_tes\_relaxed} 能够实现低价充冷、高价放冷，并在 signed valve ramp 约束下保持零 ramp violation。EnergyPlus Runtime API 在线结果验证了 \texttt{TES\_Set} 和 \texttt{Chiller\_T\_Set} 的执行器写入链路，但由于温度安全指标尚未通过，EnergyPlus 在线矩阵仅作为 I/O coupling diagnostic，不作为最终节能收益证据。

\textbf{关键词：} 数据中心冷却；蓄冷系统；光伏消纳；分时电价；模型预测控制；连续松弛调度
\end{abstract}
...
This thesis develops a Kim-lite control-oriented framework for data-center chiller-plant and thermal-energy-storage scheduling under photovoltaic generation and time-of-use electricity prices. The main experiments use relaxed continuous plant-dispatch weights to approximate an aggregated chiller plant with modulation capability, and focus on TES scheduling under TOU, critical-peak, signed-valve-ramp, and peak-cap scenarios. Reproducible proxy results show that `paper_like_mpc_tes_relaxed` shifts cooling production from high-price periods to low-price periods while maintaining SOC and signed-valve-ramp diagnostics. EnergyPlus Runtime API results verify actuator writes for `TES_Set` and `Chiller_T_Set`, but are treated as I/O coupling diagnostics rather than final cost-saving evidence because the current online matrix does not yet satisfy the temperature-safety criterion.

\textbf{Keywords:} data center cooling; thermal energy storage; photovoltaic self-consumption; time-of-use tariff; model predictive control; relaxed plant dispatch
...
本文的研究目标是：面向南京数据中心冷却系统，建立包含 TES、PV 和 TOU 电价的 Kim-lite 控制导向模型，设计采用连续松弛冷站调度的 MPC 控制器，并通过多场景闭环仿真评估 TES 在运行成本、尖峰电价和峰值约束下的边际价值。
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item 将 TES SOC 限制、signed valve ramp、PV 弃电和电网购电成本纳入 relaxed MPC 优化问题。
  \item 设计 no-TES baseline、rule-based TES 和 relaxed TES-MPC 对比实验。
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item 构建了考虑 SOC 约束、PV 弃电、分时电价和 signed valve ramp 的 relaxed TES-MPC 控制器。
  \item 通过 baseline 对比和多场景闭环仿真，分析了 TES-MPC 控制策略的经济性、鲁棒性和适用边界。
\end{enumerate}

\section{论文结构安排}

第 1 章介绍研究背景、问题动机和本文贡献。第 2 章综述数据中心冷却、TES 技术和 MPC 控制相关研究。第 3 章建立数据中心冷却蓄冷系统的控制导向模型。第 4 章给出 relaxed MPC 控制方法。第 5 章设计实验场景和评价指标。第 6 章给出当前闭环仿真结果并讨论其边界。第 7 章总结研究结论、局限性和后续工作。
...
本文的研究定位不是提出全新的优化算法，而是建立一个边界清楚、公式完整、实验可复现的 TES-PV-TOU relaxed MPC 控制框架，并用该框架解释数据中心冷却蓄冷调度的机制和边界。
...
本阶段论文主线采用 Kim-lite relaxed control-oriented model。该模型用于分析冷站--TES 在分时电价、尖峰电价和峰值约束下的调度机制，而不是严格复现单台冷机开停的整数调度。主结果中的冷站模式变量采用连续松弛处理，用于近似聚合冷站的连续调节能力；严格整数模式选择保留为后续工作。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llll}
    ...
    控制变量 & $Q_{chiller,k}$ & 冷站供冷量 & kW \\
    控制变量 & $Q_{TES,k}^{net}$ & TES 净充冷功率 & kW \\
    控制变量 & $P_{grid,k}$ & 电网购电功率 & kW \\
    控制变量 & $P_{spill,k}$ & PV 弃电功率 & kW \\
    连续松弛变量 & $\rho_{j,k}$ & 冷站模式分配权重 & - \\
...
在 Kim-lite 主结果中，TES 净热功率由冷站供冷与外生冷负荷之差定义：

\begin{equation}
  Q_{TES,k}^{net}=Q_{chiller,k}-Q_{load,k}.
\end{equation}

其中 $Q_{TES,k}^{net}>0$ 表示 TES 充冷，$Q_{TES,k}^{net}<0$ 表示 TES 放冷。该 signed-net 表述避免了同时充放冷变量的额外整数互斥约束，并便于定义归一化 signed valve proxy。
...
主实验不再把充放冷互斥写成二进制变量，而是通过 $Q_{TES,k}^{net}$ 的符号区分充冷和放冷。冷站模式权重 $\rho_{j,k}$ 在主结果中连续松弛，用于近似聚合冷站调节能力。
...
\chapter{基于 Relaxed MPC 的控制方法设计}
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item 求解未来 $N$ 步的 relaxed MPC 优化问题。
  \item 只执行第一步控制动作。
...
\begin{equation}
  \mathbf{u}
  =
  \{Q_{chiller,k},Q_{TES,k}^{net},P_{grid,k},P_{spill,k},\rho_{j,k},s_{T,k}\}_{k=0}^{N-1}.
\end{equation}

状态变量 $SOC_k$ 和 $T_k$ 通过动态方程在预测窗口内递推。本文主结果采用 LP-relaxed plant dispatch formulation，即 $\rho_{j,k}$ 为连续模式分配权重。该处理降低了求解复杂度，并将分析重点放在 TES 调度机制上；它不等价于严格的单机启停整数优化。
...
其中第一项最小化购电成本；第二项惩罚 PV 弃电；第三项避免 TES 过度循环；温度松弛项用于记录和惩罚温度风险；动作变化项用于限制 signed valve proxy 的变化率；最后一项用于约束终端 SOC 或终端偏差。Kim-lite 主结果中的 SOC 范围为硬约束，不再用 SOC slack 生成看似可行的越界结果。
...
\begin{longtable}{lll}
  ...
  TES 净功率限制 & $-Q_{dis}^{max}\leq Q_{TES,k}^{net}\leq Q_{ch}^{max}$ & 设备能力限制 \\
  模式权重 & $0\leq \rho_{j,k}\leq 1,\sum_j\rho_{j,k}\leq 1$ & relaxed plant dispatch \\
...
主结果采用连续松弛的冷站模式分配模型。该模型保留冷站功率、冷负荷、TES SOC、PV 弃电和电网功率之间的主要关系，但不严格描述单台冷机的开停整数逻辑。结果文件中保留 \texttt{mode\_integrality}、\texttt{mode\_fractionality\_max}、\texttt{mode\_fractionality\_mean}、\texttt{mode\_fractionality\_count} 和 \texttt{mode\_fractionality\_hours}，用于说明 relaxed dispatch 对结果解释的影响。严格二进制模式选择作为后续工作。
...
控制层已在顶层 \texttt{mpc\_v2} 目录中使用 Python 实现，求解器为 SciPy/HiGHS。输入包括南京 PV、TOU 电价、合成室外温度和 IT 负荷时间序列；输出包括逐时刻控制动作、状态轨迹、求解日志和指标汇总。当前主结果是 Kim-lite relaxed proxy 验证；EnergyPlus 在线结果仅作为 I/O coupling diagnostic。
...
本文设置 direct no-TES、price-aware no-TES MPC、rule-based TES 和 \texttt{paper\_like\_mpc\_tes\_relaxed} 四类 baseline / controller，以拆分冷站经济调度和 TES 的增量贡献：
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lll}
    ...
    MPC no-TES & 使用同一 relaxed MPC 和价格/PV/grid 目标，但禁用 TES & 归因冷站经济调度 \\
    rule-based TES & 按固定低价/高价时段充放冷 & 简单规则对照 \\
    paper-like MPC TES relaxed & 根据预测信息滚动优化冷站与 TES & 本文主方法 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\section{Relaxed MPC 控制器设置}

MPC 控制器每 15 min 求解一次 12 h relaxed optimization 问题，只执行第一步动作。求解器状态、求解时间、\texttt{mode\_integrality} 和 mode fractionality 诊断必须记录。若求解失败，应采用保守 fallback，并记录 infeasible 或 solver failure 次数。
...
MPC 需要在每个控制步反复求解线性或连续松弛优化问题，因此求解性能是工程可行性的重要指标。当前默认测试不再使用 192-step horizon，而是采用较短 receding horizon，使 \texttt{pytest -q} 能快速完成。48 h prediction horizon 只保留为 slow/manual 扩展实验，不能写成最终候选 horizon。
...
本版本最重要的边界是：主经济性结论来自 Kim-lite relaxed proxy，而不是 EnergyPlus 在线矩阵。该 proxy 中冷负荷外生，no-TES MPC 与 direct no-TES 的差异有限；TES 的价值主要体现在低价充冷、高价放冷、critical peak 行为变化和部分 peak-cap slack reduction。由于采用 relaxed plant dispatch，结果更接近理想化连续冷站调节能力，而非严格单机启停调度。

\chapter{结论与展望}

\section{主要研究结论}

本文建立了面向数据中心冷却系统的冷机 + TES-PV-TOU 控制导向模型，并构建了 Kim-lite relaxed MPC 控制器。该控制器能够在滚动预测时域内优化冷站供冷量、TES 净充放冷、购电功率和 PV 弃电，同时保持 TES SOC 约束和 signed valve ramp 约束。

当前 Kim-lite proxy 结果表明，\texttt{paper\_like\_mpc\_tes\_relaxed} 能够实现低价充冷、高价放冷，并在 Phase B attribution 中相对 \texttt{mpc\_no\_tes} 产生有限但可解释的 TES 边际价值。TOU/critical peak 敏感性结果显示，峰谷价差增大时 TES 价值上升，显式 critical peak 窗口会改变 TES 放冷时段。Peak-cap 结果需要按 slack 指标解释：TES 能在部分约束下减少峰值违约电量，但在更紧 cap 下不保证降低最大瞬时违约。因此，本文主结论应聚焦于 relaxed proxy 层的 TES 调度机制和敏感性，而不是严格冷机启停优化。

EnergyPlus Runtime API 在线结果在本文中定位为 I/O coupling diagnostic。现有矩阵表明 \texttt{TES\_Set} 与 \texttt{Chiller\_T\_Set} actuator 写入和 echo 校验成功，且闭环可运行；但由于 MPC rows 相对同季节 \texttt{no\_mpc} 存在温度恶化，\texttt{cost\_comparison\_valid=false} 的结果不能作为最终节能收益证据。
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item 主结果采用 relaxed plant dispatch，代表聚合冷站连续调节近似，不严格描述单台冷机开停整数逻辑。
  \item EnergyPlus 在线结果当前仅用于验证 I/O 耦合和暴露温度安全风险，不作为最终节能收益结论。
...
\begin{enumerate}[label=(\arabic*)]
  \item 在 EnergyPlus 闭环中加入更强的温度安全约束和控制面，提升在线验证的工程可信度。
  \item 扩展严格整数冷机模式选择，用于对比 relaxed dispatch 对经济性估计的影响。
...
项目前期曾考虑基于强化学习的数据中心冷却控制路线。但数据中心冷却系统具有强安全约束、真实系统试错成本高、训练稳定性要求高等特点。结合毕业设计周期和可验证性要求，本文将主线调整为基于模型的 relaxed MPC 控制方法。旧 DRL、Sinergym 和失败实验不进入正文主线，仅作为方法选择背景保留。
