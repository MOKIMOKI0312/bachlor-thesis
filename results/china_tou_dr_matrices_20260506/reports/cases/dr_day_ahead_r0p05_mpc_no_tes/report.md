# dr_day_ahead_r0p05_mpc_no_tes

- Category: `DR event`
- Raw run directory: `results\china_tou_dr_matrices_20260506\raw\dr_day_ahead_r0p05_mpc_no_tes`
...
| Steps | 2880 |
| Total cost CNY | 1,303,197.23 |
| Grid import kWh | 14,285,773.30 |
| Peak grid kW | 21,090.00 |
| Temp violation degree-hours | 0.0000 |
| Fallback count | 0 |
| Solve time p95 s | 0.1653 |
| Final SOC | 0.5000 |
...
- 该 case 的月总成本为 1,303,197.23 CNY，峰值购电为 21,090.00 kW。
- 温度违约为 0，当前代理模型下满足温度约束。
- 求解过程中未触发 fallback。
- 与配对场景 `dr_day_ahead_r0p05_mpc_tes` 相比，`mpc_no_tes -> mpc` 的 TES 增量为 节省 152.74 CNY/月。
...
|:-----------|:------------------------------|:-------------|:-------------------------------------------|----------------------:|--------------------------:|-----------------------:|----------------:|-----------------------------:|--------------------------:|-----------------:|
| dr_event   | dr_day_ahead_r0p05_mpc_no_tes | day_ahead    | 2025-07-20 18:00:00 to 2025-07-20 19:45:00 |                 38000 |                      1900 |                2060.78 |         1.08462 |                            0 |                         0 |                0 |
