# dr_day_ahead_r0p05_mpc_tes

- Category: `DR event`
- Raw run directory: `results\china_tou_dr_matrices_20260506\raw\dr_day_ahead_r0p05_mpc_tes`
...
| Controller | mpc |
| Steps | 2880 |
| Total cost CNY | 1,303,044.50 |
| Grid import kWh | 14,282,641.81 |
| Peak grid kW | 21,090.00 |
| Temp violation degree-hours | 0.0000 |
| Fallback count | 0 |
| Solve time p95 s | 0.3768 |
| Final SOC | 0.1559 |
| TES charge kWh_th | 89,152.56 |
| TES discharge kWh_th | 76,399.19 |
| DR event count | 1 |

## Analysis

- 该 case 的月总成本为 1,303,044.50 CNY，峰值购电为 21,090.00 kW。
- 温度违约为 0，当前代理模型下满足温度约束。
- 求解过程中未触发 fallback。
- 与配对场景 `dr_day_ahead_r0p05_mpc_no_tes` 相比，`mpc_no_tes -> mpc` 的 TES 增量为 节省 152.74 CNY/月。
- DR 事件数为 1，请求削减 1,900.00 kWh，达成削减 2,060.78 kWh。
...
|:-----------|:---------------------------|:-------------|:-------------------------------------------|----------------------:|--------------------------:|-----------------------:|----------------:|-----------------------------:|--------------------------:|-----------------:|
| dr_event   | dr_day_ahead_r0p05_mpc_tes | day_ahead    | 2025-07-20 18:00:00 to 2025-07-20 19:45:00 |                 38000 |                      1900 |                2060.78 |         1.08462 |                       2625.4 |                         0 |                0 |
