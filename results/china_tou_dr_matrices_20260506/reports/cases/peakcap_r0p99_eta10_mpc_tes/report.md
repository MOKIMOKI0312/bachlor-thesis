# peakcap_r0p99_eta10_mpc_tes

- Category: `Peak-cap`
- Raw run directory: `results\china_tou_dr_matrices_20260506\raw\peakcap_r0p99_eta10_mpc_tes`
...
| Controller | mpc |
| Steps | 2880 |
| Total cost CNY | 1,307,783.14 |
| Grid import kWh | 14,276,833.37 |
| Peak grid kW | 20,879.10 |
| Temp violation degree-hours | 0.0000 |
| Fallback count | 0 |
| Solve time p95 s | 0.5868 |
| Final SOC | 0.1522 |
| TES charge kWh_th | 26,406.49 |
| TES discharge kWh_th | 24,297.60 |
| DR event count | 0 |

## Analysis

- 该 case 的月总成本为 1,307,783.14 CNY，峰值购电为 20,879.10 kW。
- 温度违约为 0，当前代理模型下满足温度约束。
- 求解过程中未触发 fallback。
- 与配对场景 `peakcap_r0p99_eta10_mpc_no_tes` 相比，`mpc_no_tes -> mpc` 的 TES 增量为 节省 178.91 CNY/月。
