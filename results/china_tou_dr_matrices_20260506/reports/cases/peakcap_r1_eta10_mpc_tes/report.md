# peakcap_r1_eta10_mpc_tes

- Category: `Peak-cap`
- Raw run directory: `results\china_tou_dr_matrices_20260506\raw\peakcap_r1_eta10_mpc_tes`
...
| Controller | mpc |
| Steps | 2880 |
| Total cost CNY | 1,302,890.14 |
| Grid import kWh | 14,283,102.31 |
| Peak grid kW | 21,090.00 |
| Temp violation degree-hours | 0.0000 |
| Fallback count | 0 |
| Solve time p95 s | 0.3723 |
| Final SOC | 0.1559 |
| TES charge kWh_th | 81,890.66 |
| TES discharge kWh_th | 70,232.89 |
| DR event count | 0 |

## Analysis

- 该 case 的月总成本为 1,302,890.14 CNY，峰值购电为 21,090.00 kW。
- 温度违约为 0，当前代理模型下满足温度约束。
- 求解过程中未触发 fallback。
- 与配对场景 `peakcap_r1_eta10_mpc_no_tes` 相比，`mpc_no_tes -> mpc` 的 TES 增量为 节省 770.38 CNY/月。
