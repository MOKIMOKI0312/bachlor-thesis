# peakcap_r1_eta20_mpc_no_tes

- Category: `Peak-cap`
- Raw run directory: `results\china_tou_dr_matrices_20260506\raw\peakcap_r1_eta20_mpc_no_tes`
...
| Steps | 2880 |
| Total cost CNY | 1,303,635.01 |
| Grid import kWh | 14,283,887.85 |
| Peak grid kW | 21,090.00 |
| Temp violation degree-hours | 0.0000 |
| Fallback count | 0 |
| Solve time p95 s | 0.1990 |
| Final SOC | 0.5000 |
...
- 该 case 的月总成本为 1,303,635.01 CNY，峰值购电为 21,090.00 kW。
- 温度违约为 0，当前代理模型下满足温度约束。
- 求解过程中未触发 fallback。
- 与配对场景 `peakcap_r1_eta20_mpc_tes` 相比，`mpc_no_tes -> mpc` 的 TES 增量为 节省 744.87 CNY/月。
