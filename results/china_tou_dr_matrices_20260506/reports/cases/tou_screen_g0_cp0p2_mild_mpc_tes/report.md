# tou_screen_g0_cp0p2_mild_mpc_tes

- Category: `TOU screening`
- Raw run directory: `results\china_tou_dr_matrices_20260506\raw\tou_screen_g0_cp0p2_mild_mpc_tes`
...
| Controller | mpc |
| Steps | 2880 |
| Total cost CNY | 1,247,255.43 |
| Grid import kWh | 13,680,355.52 |
| Peak grid kW | 21,090.00 |
| Temp violation degree-hours | 0.0000 |
| Fallback count | 0 |
| Solve time p95 s | 0.4369 |
| Final SOC | 0.1534 |
| TES charge kWh_th | 11,752.40 |
| TES discharge kWh_th | 11,838.06 |
| DR event count | 0 |

## Analysis

- 该 case 的月总成本为 1,247,255.43 CNY，峰值购电为 21,090.00 kW。
- 温度违约为 0，当前代理模型下满足温度约束。
- 求解过程中未触发 fallback。
- 与配对场景 `tou_screen_g0_cp0p2_mild_mpc_no_tes` 相比，`mpc_no_tes -> mpc` 的 TES 增量为 增加成本 427.71 CNY/月。
