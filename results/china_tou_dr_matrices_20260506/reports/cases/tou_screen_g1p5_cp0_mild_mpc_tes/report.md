# tou_screen_g1p5_cp0_mild_mpc_tes

- Category: `TOU screening`
- Raw run directory: `results\china_tou_dr_matrices_20260506\raw\tou_screen_g1p5_cp0_mild_mpc_tes`
...
| Controller | mpc |
| Steps | 2880 |
| Total cost CNY | 1,228,555.20 |
| Grid import kWh | 13,718,811.58 |
| Peak grid kW | 21,090.00 |
| Temp violation degree-hours | 0.0000 |
| Fallback count | 0 |
| Solve time p95 s | 0.8514 |
| Final SOC | 0.1500 |
| TES charge kWh_th | 100,511.58 |
| TES discharge kWh_th | 86,244.05 |
| DR event count | 0 |

## Analysis

- 该 case 的月总成本为 1,228,555.20 CNY，峰值购电为 21,090.00 kW。
- 温度违约为 0，当前代理模型下满足温度约束。
- 求解过程中未触发 fallback。
- 与配对场景 `tou_screen_g1p5_cp0_mild_mpc_no_tes` 相比，`mpc_no_tes -> mpc` 的 TES 增量为 增加成本 97.77 CNY/月。
