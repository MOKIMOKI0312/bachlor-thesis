# tou_screen_g2_cp0_hot_mpc_tes

- Category: `TOU screening`
- Raw run directory: `results\china_tou_dr_matrices_20260506\raw\tou_screen_g2_cp0_hot_mpc_tes`
...
| Controller | mpc |
| Steps | 2880 |
| Total cost CNY | 1,433,446.67 |
| Grid import kWh | 14,281,226.23 |
| Peak grid kW | 21,090.00 |
| Temp violation degree-hours | 0.0000 |
| Fallback count | 0 |
| Solve time p95 s | 0.3132 |
| Final SOC | 0.1559 |
| TES charge kWh_th | 90,709.29 |
| TES discharge kWh_th | 77,725.48 |
| DR event count | 0 |

## Analysis

- 该 case 的月总成本为 1,433,446.67 CNY，峰值购电为 21,090.00 kW。
- 温度违约为 0，当前代理模型下满足温度约束。
- 求解过程中未触发 fallback。
- 与配对场景 `tou_screen_g2_cp0_hot_mpc_no_tes` 相比，`mpc_no_tes -> mpc` 的 TES 增量为 节省 710.23 CNY/月。
